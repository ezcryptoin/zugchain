 ...
 *    Enhanced with Wallet Generator & Manual Referral Input
 * ===============================================
 */
...
async function createWallets() {
    ...
    // --- MANUAL REFERRAL INPUT (HANYA INI YANG DITAMBAHKAN) ---
    const referralInput = await askQuestion(
        chalk.yellow('Enter your referral code (default: ZUG-J11YWX70): ')
    );
    const referralCode = referralInput.trim() || 'ZUG-J11YWX70';
    console.log(chalk.green(`✓ Using referral code: ${referralCode}`));
    // -----------------------------------------------------------
    ...
    const wallets = [];
    console.log(chalk.green(`\nGenerating ${count} wallets with referral: ${referralCode}\n`));
    ...
    // Sync with referral system menggunakan referral code dari input user
    ...
    for (let i = 0; i < wallets.length; i++) {
        try {
            ...
            await client.syncUser(referralCode);  // Gunakan referral code dari input user
    ...
    console.log(chalk.bold.cyan(`\n${syncedCount}/${count} wallets successfully registered with referral ${referralCode}!`));
    ...
    if (runBot.toLowerCase() === 'y') {
        // Inject referral code ke config global
...
async function runWithManualKeys() {
    ...
    // --- MANUAL REFERRAL INPUT UNTUK FAUCET (HANYA INI YANG DITAMBAHKAN) ---
    const referralInput = await askQuestion(
        chalk.yellow('Enter referral code for faucet claims (default: ZUG-J11YWX70): ')
    );
    const referralCode = referralInput.trim() || 'ZUG-J11YWX70';
    console.log(chalk.green(`✓ Using referral code: ${referralCode}\n`));
    // -----------------------------------------------------------------------
    ...
    // Inject referral code ke config global sebelum menjalankan bot
    config.referralCode = referralCode;
