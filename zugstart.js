 ...
 *    Enhanced with Manual Referral Input & Secure Config
 * ===============================================
 * 
 * 🔒 SECURITY FEATURES:
 * - config.json TIDAK mengandung private key (aman di-commit ke GitHub)
 * - 2Captcha API Key HANYA di accounts.json (hidden, tidak ditampilkan)
 * - Private key hanya di accounts.json (HARUS di-.gitignore)
 * - Referral code diinput manual saat runtime
 */
...
// 🔒 BACA API KEY DARI accounts.json SAJA (TIDAK ADA INPUT PROMPT)
try {
...
class ZugChainClient {
    ...
    getHeaders() {
        return {
            ...
            'origin': config.baseUrl.trim(),
            'pragma': 'no-cache',
            'referer': `${config.baseUrl.trim()}/mission-control`,
    ...
    async claimFaucet(missions = []) {
        try {
            ...
            const FAUCET_PAGE = `${config.baseUrl.trim()}/faucet`;
            const captchaToken = await this.solveCaptcha(SITE_KEY, FAUCET_PAGE);
            
            // 🔑 Referral code dari input user (bukan dari config)
            const referralCode = global.userReferralCode || config.referralCode || 'ZUG-J11YWX70';
...
class AccountScheduler {
    ...
    async run() {
        ...
        try {
            ...
            try {
                // 🔑 Gunakan referral code dari input user
                await this.client.syncUser(global.userReferralCode || '');
            } catch (e) { }
            ...
            // ✅ MENYELESAIKAN SOCIAL TASKS (Twitter/X, Telegram, Discord, dll)
            for (const mission of missions) {
                ...
                // ✅ HANDLE SOCIAL TASKS (LINK_CLICK)
...
class ZugChainBot {
    ...
    async initialize() {
        ...
        // 🔒 VALIDASI API KEY DI accounts.json
        if (!twoCaptchaApiKey || twoCaptchaApiKey.trim() === '') {
            console.log(chalk.red('[!] ERROR: 2Captcha API Key not found in accounts.json!'));
            console.log(chalk.yellow('[i] Please add "twoCaptchaApiKey" field to accounts.json'));
            console.log(chalk.yellow('[i] Example: { "twoCaptchaApiKey": "YOUR_API_KEY_HERE", "accounts": [...] }'));
            process.exit(1);
        }
        ...
        console.log(chalk.green(`✓ 2Captcha API key loaded (hidden for security)`));
        console.log();
...
// ============ MENU SYSTEM WITH MANUAL REFERRAL INPUT ============
...
async function showMainMenu() {
    ...
    console.log(chalk.white('1.') + ' Create random wallets (max 5) with CUSTOM referral');
...
async function createWallets() {
    ...
    // 🔑 INPUT REFERRAL CODE MANUAL (TIDAK DISIMPAN DI CONFIG)
    const referralInput = await askQuestion(
        chalk.yellow('Enter your referral code (press Enter for default ZUG-J11YWX70): ')
    );
    global.userReferralCode = referralInput.trim() || 'ZUG-J11YWX70';
    console.log(chalk.green(`✓ Using referral code: ${global.userReferralCode}`));
    ...
    const wallets = [];
    console.log(chalk.green(`\nGenerating ${count} wallets with referral: ${global.userReferralCode}\n`));
    ...
    for (let i = 0; i < count; i++) {
        ...
        console.log(chalk.yellowBright(`Private Key: ${wallet.privateKey}`));
        console.log(chalk.cyan(`Address: ${wallet.address}`));
        console.log(chalk.white(`Referral Link: https://testnet.zugchain.org/?ref=${global.userReferralCode}`));
        console.log();
    }

    // Save to file (accounts.json - HARUS DI .gitignore)
    ...
    try {
        ...
        console.log(chalk.yellow(`⚠️  REMEMBER: Add generated_accounts.json to .gitignore!`));
    } catch (e) {
    ...
    for (let i = 0; i < wallets.length; i++) {
        try {
            ...
            await client.syncUser(global.userReferralCode);
    ...
    console.log(chalk.bold.cyan(`\n${syncedCount}/${count} wallets successfully registered with referral ${global.userReferralCode}!`));
    console.log(chalk.yellowBright('IMPORTANT: Save your private keys securely! They will not be shown again.'));
...
async function runWithManualKeys() {
    ...
    // 🔑 INPUT REFERRAL CODE MANUAL
    const referralInput = await askQuestion(
        chalk.yellow('Enter referral code for faucet claims (press Enter for default ZUG-J11YWX70): ')
    );
    global.userReferralCode = referralInput.trim() || 'ZUG-J11YWX70';
    console.log(chalk.green(`✓ Using referral code: ${global.userReferralCode}\n`));
    ...
    // ✅ TIDAK ADA INPUT API KEY DI SINI - HANYA DI accounts.json
    console.log(chalk.green(`✓ Accounts loaded. 2Captcha API key will be read from accounts.json`));
    
    await runBotWithAccounts(accountsArray);
}

async function runBotWithAccounts(accountList) {
    accounts = accountList;
    
    console.log(chalk.green(`\n✓ Starting bot with ${accounts.length} account(s)...`));
    console.log(chalk.yellow(`✓ Active referral code: ${global.userReferralCode || 'ZUG-J11YWX70'}`));
    console.log(chalk.green(`✓ 2Captcha API key loaded from accounts.json (hidden for security)`));
...
⚠️  SECURITY REMINDER:
   - config.json is SAFE to commit to GitHub (no private keys!)
   - 2Captcha API Key is ONLY in accounts.json (HIDDEN, not displayed)
   - Private keys are ONLY in accounts.json (MUST be in .gitignore)
   - Referral code is input manually at runtime
   
📝 SETUP INSTRUCTIONS:
   1. Edit accounts.json and add your 2Captcha API key:
      {
        "twoCaptchaApiKey": "YOUR_API_KEY_HERE",
        "accounts": []
      }
   2. Add accounts.json to .gitignore
